...
/**
 * Compares tasks by their (type, target) content only — ignores the object
 * keys Discord assigns and their order, so a key/ordering-only difference
 * never counts as a real change. Used by both detectQuestChanges (to decide
 * IF tasks changed) and buildChangeDescription's task summary (to show
 * WHAT changed) — sharing this basis is what guarantees that whenever
 * detection says "changed", the displayed old/new text is actually
 * different too (previously the summary only used task.type, so a
 * target/duration-only change was detected but rendered as an identical
 * "~~X~~ → X" line).
 */
function taskSignature(tasks) {
  return Object.values(tasks || {})
    .map(t => `${t?.type || ''}:${t?.target || 0}`)
    .sort()
    .join('|');
}
...
function summarizeTasks(tasks) {
  return (
    Object.values(tasks || {})
      .map(t => {
        const name = String(t?.type || '').replace(/_/g, ' ');
...
export function detectQuestChanges(oldQuest, newQuest) {
  ...
  if (taskSignature(oldTasks) !== taskSignature(newTasks)) {
    changes.tasks = true;
  }
...
export function buildChangeDescription(oldQuest, newQuest, changes) {
  ...
  if (changes?.tasks) {
    const oldT = summarizeTasks(oldConfig.task_config_v2?.tasks);
    const newT = summarizeTasks(newConfig.task_config_v2?.tasks);
